...
Potree.PointCloudQuadtree = class extends Potree.PointCloudTree{
	...
	constructor(geometry, material){
		...
		this.material = material || new Potree.PointCloudMaterial({ treeType: Potree.TreeType.QUADTREE });
	...
	updateVisibilityTexture(material, visibleNodes){
		...
		for(let i = 0; i < visibleNodes.length; i++){
			...
			for(let j = 0; j < 4; j++){
				let child = node.children[j];
				if(child instanceof Potree.PointCloudQuadtreeNode && child.sceneNode.visible && visibleNodes.indexOf(child) >= 0){
					children.push(child);
				}
			}
