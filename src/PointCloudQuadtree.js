...
Potree.PointCloudQuadtreeNode = class PointCloudQuadtreeNode extends Potree.PointCloudTreeNode {
	constructor () {
		super();
...
Potree.PointCloudQuadtree = class extends Potree.PointCloudTree {
	...
	toTreeNode (geometryNode, parent) {
		let node = new Potree.PointCloudQuadtreeNode();
	...
	updateVisibleBounds () {
		...
		for (let i = 0; i < this.visibleNodes.length; i++) {
			...
			for (let j = 0; j < node.children.length; j++) {
				...
				if (child instanceof Potree.PointCloudQuadtreeNode) {
					isLeaf = isLeaf && !child.sceneNode.visible;
				} else if (child instanceof Potree.PointCloudQuadtreeGeometryNode) {
					isLeaf = true;
				}
			}
	...
	computeVisibilityTextureData(nodes, camera){
		...
		for (let i = 0; i < nodes.length; i++) {
			...
			for (let j = 0; j < 4; j++) {
				...
				if( child && child.constructor === Potree.PointCloudQuadtreeNode && nodes.includes(child, i)){
					children.push(child);
				}
			}
	 ...
	 *
	 * maxDepth:		search points up to the given quadtree depth
	 *
	 *
		...
		for (let i = 0; i < points.segments.length; i++) {
			...
			let project = (function (_start, _end, _mileage, _boundingBox) {
				...
				return function (position) {
					let toOrigin = new THREE.Matrix4().makeTranslation(-start.x, -boundingBox.center.y, -start.z);
	 ...
	 * end:
	 * width:
	 * depth:		search points up to the given quadtree depth
	...
	pick(viewer, camera, ray, params = {}){
		...
		{ // RENDER
			...
			pRenderer.renderQuadtree(this, nodes, camera, pickState.renderTarget);
